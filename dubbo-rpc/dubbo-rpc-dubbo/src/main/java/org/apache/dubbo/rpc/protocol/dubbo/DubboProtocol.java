...
public class DubboProtocol extends AbstractProtocol {
	...
	private ExchangeHandler requestHandler = new ExchangeHandlerAdapter() {
			...
			if (Boolean.TRUE.toString().equals(inv.getAttachments().get(IS_CALLBACK_SERVICE_INVOKE))) {
				...
				if (!hasMethod) {
					logger.warn(new IllegalStateException("The methodName " + inv.getMethodName()
							...
							+ " please update the api interface. url is:" + invoker.getUrl()) + " ,invocation is :"
							+ inv);
					return null;
				}
			}
		...
		private void invoke(Channel channel, String methodKey) {
			...
			if (invocation != null) {
				try {
					...
					logger.warn("Failed to invoke event method " + invocation.getMethodName() + "(), cause: "
							+ t.getMessage(), t);
				}
			}
		}
		...
		/**
		 * FIXME channel.getUrl() always binds to a fixed service, and this service is
		 * random. we can choose to use a common service to carry onConnect event if
		 * there's no easy way to get the specific service this connection is binding
		 * to.
		 * 
		 * @param channel
		 * @param url
		...
		private Invocation createInvocation(Channel channel, URL url, String methodKey) {
			...
			RpcInvocation invocation = new RpcInvocation(method, url.getParameter(INTERFACE_KEY), new Class<?>[0],
					new Object[0]);
	...
	Invoker<?> getInvoker(Channel channel, Invocation inv) throws RemotingException {
		...
		String serviceKey = serviceKey(port, path, (String) inv.getAttachments().get(VERSION_KEY),
				(String) inv.getAttachments().get(GROUP_KEY));
	...
	private List<ReferenceCountExchangeClient> getSharedClient(URL url, int connectNum) {
		...
		synchronized (locks.get(key)) {
			...
			/*
			 * I understand that the purpose of the remove operation here is to avoid the
			 * expired url key always occupying this memory space.
			 */
			locks.remove(key);
	...
	private ExchangeClient initClient(URL url) {
		...
		if (str != null && str.length() > 0
				&& !ExtensionLoader.getExtensionLoader(Transporter.class).hasExtension(str)) {
			throw new RpcException("Unsupported client type: " + str + "," + " supported client type is " + StringUtils
	...
	private void closeReferenceCountExchangeClient(ReferenceCountExchangeClient client) {
		...
		try {
			 ...
			 * LazyConnectExchangeClient. Do you need to call client.close again to ensure
			 * that LazyConnectExchangeClient is also closed?
			 */
